function [U,L,global_para]=standard_temp_calculation(U,L,global_para)
%% 参数释放和部分参数求解
...
hole_type=global_para.temp_calculation.hole_type;       % 单孔进油/周向油槽
...
p_lim=50E6;    % 比压极限值/Pa
T_lim=75;      % 轴承极限温度值/°
h_lim=9E-6;    % 最小油膜厚度值/m

cal_type=2;    % 传导散热-1/对流散热-2

% Angle=360-hole_angle;
...
RS=RS/60;
%% 雷诺数
C_Reff=1.85E-3*Dbearing;
eta_eff=vis(T_envir);
Re=DEN*pi*Dbearing*RS*C_Reff/2/eta_eff;
if Re>41.3*sqrt(Dbearing/C_Reff)
    errordlg('Temperature Calculation:No laminar flow','Program wrong');
end
%% 轴承比压数值
p_bar=W/Dbearing/Length;
if p_bar>p_lim
    errordlg('Temperature Calculation:Overload','Program wrong');
end
%% 
% [T_B,h_min,So,beta,~,Q,psi_eff,psi_bar,epsilon,f,global_para]...
%     =iteration_new(inlet_temp,min_gap,max_gap,Dbearing,line_expand_coef_bearing,line_expand_coef_shaft,...
%     RS,0,Length,W,T_envir,k_A,A,hole_diameter,pressure_inlet,cal_type,hole_type,global_para);

[T_B,h_min,So,beta,~,Q,psi_eff,psi_bar,epsilon,f,global_para]...
    =iteration_new(inlet_temp,Dbearing,...
    RS,0,Length,CR,W,T_envir,hole_diameter,pressure_inlet,cal_type,hole_type,global_para);
